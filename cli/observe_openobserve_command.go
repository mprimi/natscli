...
func configureOpenObserveCommand(srv *fisk.CmdClause) {
	...
	// Example: observe openobserve -u me@example.com -p 1234 -a http://localhost:5080/api/default/ingest/metrics/_json
...
func createDummyMetric(dimension string, value float64) dummyMetric {
	fmt.Printf("%s: %.1f\n", dimension, value)
	return dummyMetric{
		Name:      "dummy__" + dimension,
		Type:      "gauge",
		Host:      "localhost",
...
func (c *observeOpenObserveCmd) openObserve(context *fisk.ParseContext) error {
	...
	for i := 0; i < numSamples; i++ {
		...
		fmt.Printf("Publishing sample %d of %d", i+1, numSamples)
		samples := []dummyMetric{
			createDummyMetric("CPU", float64(i*10)),
			createDummyMetric("MEM", float64(GB+MB*rng.Int31n(256))),
			createDummyMetric("LOAD", rand.Float64()),
		}
